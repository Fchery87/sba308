...
//       if (typeof submission.submission.score !== 'number' || submission.submission.score < 0) {
//         throw new Error("Invalid submission score: Score must be a non-negative number.");
//       }
//     });
//   }
...
//           const adjustedScore = lateSubmission ? submission.submission.score - (0.1 * assignment.points_possible) : submission.submission.score;
//           totalScore += adjustedScore;
//           totalPossible += assignment.points_possible;
//           weightedScores[assignment.id] = (adjustedScore / assignment.points_possible) * 100;
//         }
//       });
//     });
  
//     const weightedAverage = (totalScore / totalPossible) || 0;
...
//       const { weightedAverage, weightedScores } = calculateWeightedAverage(assignments, learnerData[learnerId]);
...
function calculateWeightedAverage(assignments, submissions) {
  ...
  assignments.forEach(assignment => {
    ...
    if (currentDate >= dueDate) {
      submissions.forEach(submission => {
        if (submission.assignment_id === assignment.id) {
          const lateSubmission = new Date(submission.submission.submitted_at) > dueDate;
          const adjustedScore = lateSubmission ? submission.submission.score - (0.1 * assignment.points_possible) : submission.submission.score;
          totalScore += adjustedScore;
          totalPossible += assignment.points_possible;
          weightedScores[assignment.id] = (adjustedScore / assignment.points_possible) * 100;
